...
class Addexperiment extends CI_Controller{
	...
	public function insert(){
		...
		$etime=$_POST['time'];
		$extime=$_POST['extime'];
		$eclutime=$_POST['eclutime'];

		$ok=DB::insert('experimentinfo',['coid'=>$coid,'ename'=>$ename,'einfo'=>$einfo,'etime'=>$etime,'extime'=>$extime,'eclutime'=>$eclutime]);
		if($ok>0){
			$this->json([
				'code'=>200,
				'msg'=>'数据插入成功',
	...
	public function update(){
		...
		$ok=DB::update('experimentinfo',['ename'=>$ename,'einfo'=>$einfo,'etime'=>$etime,'extime'=>$extime,'eclutime'=>$eclutime],['id'=>$id]);
		if($ok>0){
			$this->json([
				'code'=>200,
				'msg'=>'数据更新成功',
