		  	...
		  	<div class="grid-content-light bg-purple-light" v-show="experimentVisable">
		  		...
		  		<div class="modelcontent">
			  		<el-form :model="experiment" :rules="experimentrules" ref="experiment" label-width="100px" class="demo-ruleForm" label-position="top">
	  					...
	  					<el-form-item label="实验时长（分）" prop="time">
	    					<el-input v-model="experiment.time" placeholder="请输入实验时长" ></el-input>
	  					</el-form-item>
	  					<el-form-item label="预习测试时长（分）" prop="extime">
	    					<el-input v-model="experiment.extime" placeholder="请输入预习测试时长" ></el-input>
	  					</el-form-item>
	  					<el-form-item label="报告提交时长（天）" prop="eclutime">
	    					<el-input v-model="experiment.eclutime" placeholder="请输入报告时长" ></el-input>
	  					</el-form-item>
	  					<el-form-item>
		...
		data(){
			return {
		        ...
		        experimentrules:{
		        	...
		        	extime:[{ required: true, message: '请输入预习测试时长', trigger: 'blur' }],
		        	eclutime:[{ required: true, message: '请输入报告时长', trigger: 'blur' }],
		        },//验证信息
		        dialogImageUrl: '',
        		dialogVisible: false,
		...
		methods:{
			...
			handleNodeClick(node,data) {
		    	...
		    	if(node.level === 1){
		    		...
		    		this.experiment.extime = data.extime;
		    		this.experiment.eclutime = data.eclutime;
