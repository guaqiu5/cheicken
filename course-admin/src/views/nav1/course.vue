...
<script>
	...
	import { getUserListPage, removeUser, batchRemoveUser, editUser, addUser,getClassInfo,getProjectnameInfo,getStudentProjectInfo,getStuExreportAll,updatescore,htmltowordurl,updateopretion,getStudentAllExperment, searchTerminfo, getSearchTermClass} from '../../api/api';
	...
	export default {
		...
		methods: {
			...
			getUsers() {
				...
				//NProgress.start();
				// 班级可以不选
				if(this.filters.termname === '' || this.filters.exid === ''){
					this.$message.error('请选择');
					return;
				}
				if (this.filters.classname === '-') {
					this.filters.classname = null;
				}
			...
			handleExprement(index, row){
				...
				getStudentAllExperment(param).then((res) => {
					if(res.data.code == 200){
						const { export_json_to_excel } = require('../../vendor/Export2Excel');
						const tHeader = ['实验名称','实验预习得分','实验操作得分','学生报告得分','学生总分'];
						const filterVal = ['exname','stuChoice','stuOperation','stuReport','stuAllScore'];
						const list = res.data.data[0];
						const className = res.data.data[1]; // 防止在班级模糊查询时为null的情况出现，从数据库取出
						...
						var title = course + "—实验成绩总计(" + className + "-" + row.name +")";
			...
			getClass(value){
				...
				if(value == '-'){
					...
					getSearchTermClass(param).then(res => {
						let classInfo = Object.assign([],res.data.data);
						this.classoption.push({
							value:'-',
							label:'不限'
